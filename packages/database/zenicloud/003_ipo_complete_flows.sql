...
BEGIN
  ...
  RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS on_round_closed ON fundraise_rounds;
...
CREATE TABLE IF NOT EXISTS dd_investor_access (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid REFERENCES tenants(id) ON DELETE CASCADE NOT NULL,
  round_id uuid REFERENCES fundraise_rounds(id) ON DELETE CASCADE NOT NULL,
...
CREATE TABLE IF NOT EXISTS dd_access_logs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_dd_logs_investor ON dd_access_logs(investor_access_id);
CREATE INDEX IF NOT EXISTS idx_dd_logs_doc ON dd_access_logs(document_id);
CREATE INDEX IF NOT EXISTS idx_dd_logs_ts ON dd_access_logs(created_at DESC);

CREATE TABLE IF NOT EXISTS dd_qa_threads (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid REFERENCES tenants(id) ON DELETE CASCADE NOT NULL,
  round_id uuid REFERENCES fundraise_rounds(id),
...
CREATE TABLE IF NOT EXISTS ipo_readiness_criteria (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid REFERENCES tenants(id) ON DELETE CASCADE NOT NULL,
  journey_id uuid REFERENCES ipo_journeys(id) ON DELETE CASCADE NOT NULL,
...
CREATE TABLE IF NOT EXISTS ipo_readiness_criteria_template (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
DROP POLICY IF EXISTS academy_user_scope ON academy_progress;
...
DROP POLICY IF EXISTS public_read ON membership_tiers;
CREATE POLICY "public_read" ON membership_tiers FOR SELECT USING (true);
DROP POLICY IF EXISTS public_read ON training_drills;
...
CREATE INDEX IF NOT EXISTS idx_phase_gates_journey ON phase_gates(journey_id);
CREATE INDEX IF NOT EXISTS idx_rounds_journey ON fundraise_rounds(journey_id);
CREATE INDEX IF NOT EXISTS idx_pipeline_round ON investor_pipeline(round_id);
CREATE INDEX IF NOT EXISTS idx_pipeline_stage ON investor_pipeline(stage);
CREATE INDEX IF NOT EXISTS idx_cap_snapshots_journey ON cap_table_snapshots(journey_id, snapshot_date DESC);
CREATE INDEX IF NOT EXISTS idx_dd_access_round ON dd_investor_access(round_id);
CREATE INDEX IF NOT EXISTS idx_readiness_journey ON ipo_readiness_criteria(journey_id);
