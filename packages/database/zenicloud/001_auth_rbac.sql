...
DROP POLICY IF EXISTS profile_same_tenant_select ON user_profiles;
CREATE POLICY "profile_same_tenant_select" ON user_profiles
  FOR SELECT USING (tenant_id = public.current_tenant_id());

DROP POLICY IF EXISTS profile_self_update ON user_profiles;
CREATE POLICY "profile_self_update" ON user_profiles
  FOR UPDATE USING (id = auth.uid());

-- Only CHR/CEO can update roles within tenant
DROP POLICY IF EXISTS profile_role_admin_update ON user_profiles;
...
DROP POLICY IF EXISTS invitation_admin_create ON invitations;
CREATE POLICY "invitation_admin_create" ON invitations
  FOR INSERT WITH CHECK (
    tenant_id = public.current_tenant_id() AND public.is_chr_or_ceo()
  );

DROP POLICY IF EXISTS invitation_same_tenant_select ON invitations;
CREATE POLICY "invitation_same_tenant_select" ON invitations
  FOR SELECT USING (tenant_id = public.current_tenant_id());

-- Audit logs: read-only for same tenant
DROP POLICY IF EXISTS audit_same_tenant_read ON audit_logs;
