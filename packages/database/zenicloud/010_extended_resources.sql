...
CREATE INDEX IF NOT EXISTS idx_comparables_tenant ON comparables(tenant_id);
CREATE INDEX IF NOT EXISTS idx_comparables_industry ON comparables(industry);
ALTER TABLE comparables ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS super_admin_bypass ON comparables;
CREATE POLICY super_admin_bypass ON comparables FOR ALL USING (is_chairman_super()) WITH CHECK (is_chairman_super());
DROP POLICY IF EXISTS tenant_isolation ON comparables;
...
CREATE INDEX IF NOT EXISTS idx_market_data_tenant ON market_data(tenant_id);
CREATE INDEX IF NOT EXISTS idx_market_data_type ON market_data(tenant_id, metric_type);
ALTER TABLE market_data ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS super_admin_bypass ON market_data;
CREATE POLICY super_admin_bypass ON market_data FOR ALL USING (is_chairman_super()) WITH CHECK (is_chairman_super());
DROP POLICY IF EXISTS tenant_isolation ON market_data;
...
CREATE INDEX IF NOT EXISTS idx_market_intel_tenant ON market_intel(tenant_id);
CREATE INDEX IF NOT EXISTS idx_market_intel_severity ON market_intel(tenant_id, severity, created_at DESC);
ALTER TABLE market_intel ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS super_admin_bypass ON market_intel;
CREATE POLICY super_admin_bypass ON market_intel FOR ALL USING (is_chairman_super()) WITH CHECK (is_chairman_super());
DROP POLICY IF EXISTS tenant_isolation ON market_intel;
...
CREATE INDEX IF NOT EXISTS idx_feedback_status ON feedback_items(tenant_id, status, created_at DESC);
ALTER TABLE feedback_items ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS super_admin_bypass ON feedback_items;
CREATE POLICY super_admin_bypass ON feedback_items FOR ALL USING (is_chairman_super()) WITH CHECK (is_chairman_super());
DROP POLICY IF EXISTS tenant_isolation ON feedback_items;
...
-- ─────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS tokenomics_allocations (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid REFERENCES tenants(id) ON DELETE CASCADE NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_tokenomics_tenant ON tokenomics_allocations(tenant_id);
ALTER TABLE tokenomics_allocations ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS super_admin_bypass ON tokenomics_allocations;
CREATE POLICY super_admin_bypass ON tokenomics_allocations FOR ALL USING (is_chairman_super()) WITH CHECK (is_chairman_super());
DROP POLICY IF EXISTS tenant_isolation ON tokenomics_allocations;
...
CREATE INDEX IF NOT EXISTS idx_nlq_tenant ON nlq_logs(tenant_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_nlq_user ON nlq_logs(user_id, created_at DESC);
ALTER TABLE nlq_logs ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS super_admin_bypass ON nlq_logs;
CREATE POLICY super_admin_bypass ON nlq_logs FOR ALL USING (is_chairman_super()) WITH CHECK (is_chairman_super());
DROP POLICY IF EXISTS tenant_isolation ON nlq_logs;
...
DROP TRIGGER IF EXISTS trg_comparables_updated ON comparables;
CREATE TRIGGER trg_comparables_updated BEFORE UPDATE ON comparables
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();
DROP TRIGGER IF EXISTS trg_tokenomics_updated ON tokenomics_allocations;
