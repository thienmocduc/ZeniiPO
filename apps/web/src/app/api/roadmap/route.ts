...
export async function GET() {
  ...
  const [specsR, journeyR, canvasR, councilR, mktR, intelR, finR, okrR, sopR, schedR, boardR, govR, vaultR, readyR, roundsR, kpiR] =
    await Promise.all([
      ...
      supabase.from('financial_statements').select('period, revenue, cogs').order('period', { ascending: false }).limit(12),
      ...
      supabase.from('readiness_score_history').select('total_score, created_at').order('created_at', { ascending: false }).limit(1),
